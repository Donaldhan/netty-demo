...
import netty.initializer.math.MathClientSumInitializer;
 ...
 * 下午12:48:10
 */
public final class MathClientSum {
	private static final Logger log = LoggerFactory.getLogger(MathClientSum.class);
    ...
    private static void run() throws SSLException, InterruptedException{
        ...
        try {
            ...
            bootstrap.handler(new MathClientSumInitializer(sslCtx));
            InetSocketAddress inetSocketAddress = new InetSocketAddress(ip,port);
            //连接socket地址
            ChannelFuture f = bootstrap.connect(inetSocketAddress).sync();
            log.info("=========Sum Client is start=========");
            // Get the handler instance to retrieve the answer.
            MathClientSumHandler handler =
                (MathClientSumHandler) f.channel().pipeline().last();

...
import netty.handler.math.MathClientMultiHandler;
import netty.initializer.math.MathClientMultiInitializer;
 ...
 * 下午12:48:10
 */
public final class MathClientMulti {
	private static final Logger log = LoggerFactory.getLogger(MathClientMulti.class);
    ...
    private static void run() throws SSLException, InterruptedException{
        ...
        try {
            ...
            bootstrap.handler(new MathClientMultiInitializer(sslCtx));
            InetSocketAddress inetSocketAddress = new InetSocketAddress(ip,port);
            //连接socket地址
            ChannelFuture f = bootstrap.connect(inetSocketAddress).sync();
            log.info("=========Multi Client is start=========");
            // Get the handler instance to retrieve the answer.
            MathClientMultiHandler handler =
                (MathClientMultiHandler) f.channel().pipeline().last();
